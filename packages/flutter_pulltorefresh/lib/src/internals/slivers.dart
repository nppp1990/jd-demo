...
class RenderSliverRefresh extends RenderSliverSingleBoxAdapter {
  ...
  void performLayout() {
    ...
    if (active || (paintOffsetY != 0.0 && paintOffsetY! > 0.0)) {
      ...
      switch (refreshStyle) {
        case RefreshStyle.Follow:
          ...
          if (refreshStatus == RefreshStatus.twoLevelOpening || refreshStatus == RefreshStatus.twoLeveling) {
            originOffset = 0;
          } else {
            originOffset = paintOffsetY ?? 0.0;
          }
          geometry = SliverGeometry(
            scrollExtent: layoutExtent,
            // 相对于viewPort的位置：0表示最上边缘
            paintOrigin: -boxExtent - constraints.scrollOffset + layoutExtent + originOffset,
            ...
            // next sliver和当前sliver最开始位置的距离：这里next sliver是body
            layoutExtent: Math.min(
                needPaintExtent, Math.max(layoutExtent - constraints.scrollOffset + (paintOffsetY ?? 0.0), 0.0)),
          );
  ...
  void paint(PaintingContext paintContext, Offset offset) {
    paintContext.paintChild(child!, Offset(offset.dx, offset.dy));
  }
